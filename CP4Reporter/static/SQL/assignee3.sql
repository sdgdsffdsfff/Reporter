...
WHERE
         (pv.ID is NULL || csvr.ID is NULL)
         AND ji.ASSIGNEE in (SELECT lower_user_name FROM cwd_user cu LEFT JOIN cwd_membership cms ON cu.ID = cms.child_id where cms.parent_id = 29988)
         GROUP BY ji.id
